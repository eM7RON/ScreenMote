...
#####################################################################
# The following lines have been added to allow resource limiting in #
# order to combat the memory leak issue of uzbl browser             #
#####################################################################

import resource
from psutil import virtual_memory

# setting memory resource limits
mem = virtual_memory()
mem_soft = mem.total * 0.75
mem_hard = mem.total * 0.90
...
def generate_asset_list():
    ...
    deadlines = [
        asset["end_date"] if assets_helper.is_active(asset) else asset["start_date"]
        for asset in assets
    ]
...
def load_browser(url=None):
    ...
    browser = sh.Command("uzbl-browser")(
        print_events=True, config="-", uri=current_browser_url, _bg=True
    )
    logging.info(
        "Browser loading %s. Running as PID %s.", current_browser_url, browser.pid
    )
...
def view_video(uri, duration):
    ...
    if duration and duration != "N/A":
        player_args = [
            "timeout",
            VIDEO_TIMEOUT + int(duration.split(".")[0]),
        ] + player_args
...
def asset_loop(scheduler):
    ...
    if asset is None:
        ...
        logging.info(
            "Asset %s at %s is not available, skipping.", asset["name"], asset["uri"]
        )
        sleep(0.5)
...
def setup_hotspot():
    ...
    wireless_connections = filter(
        ...
        filter(
            lambda c: pattern_include.search(str(c["Devices"])), wireless_connections
        ),
    )
...
def main():
    ...
    url = (
        "http://{0}:{1}/splash-page".format(LISTEN, PORT)
